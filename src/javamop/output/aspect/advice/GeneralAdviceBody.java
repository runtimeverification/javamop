...
public class GeneralAdviceBody extends AdviceBody {
	...
	public GeneralAdviceBody(JavaMOPSpec mopSpec, EventDefinition event, AspectBody aspectBody) throws MOPException {
		super(mopSpec, event, aspectBody);
		...
		this.timestamp = aspectBody.timestamp;
		this.indexingTreesForCopy = aspectBody.indexingDecl.getIndexingTreesForCopy();

		if (mopSpec.has__SKIP() || event.getPos().equals("around"))
			aroundLocalDecl = new AroundAdviceLocalDecl();
		if (event.getPos().equals("around"))
			aroundAdviceReturn = new AroundAdviceReturn(event.getRetType(), event.getParameters());
		...
		enable = aspectBody.enableSet.getEnable(event.getId());
		...
		for (MOPParameters enableEntity : enable) {
			...
			if (!enableEntity.contains(param)) {
				...
				if (!param.contains(enableEntity)) {
					...
					for (MOPParameterPair paramPair : aspectBody.indexingDecl.getIndexingTreesForCopy().keySet()) {
						if (paramPair.getParam1().equals(intersectionOfEnableEntityAndParam) && paramPair.getParam2().equals(enableEntity)) {
							IndexingTree tempIndexingTree = aspectBody.indexingDecl.getIndexingTreesForCopy().get(paramPair);
