...
public class MOPMultiMapNode extends MOPMap {
	...
	public MOPMultiMapNode(MOPMultiMapSignature[] signatures) {
	...
	public void endObject(MOPMultiMapSignature[] signatures) {
		if(signatures.length != valueSize){
			System.err.println("[javamoprt] endObject of MOPMultiMapNode error.");
			return;
		}
							...
							if(values[j] != null){
								if(valuePattern[j].type == MOPMultiMapSignature.MAP_OF_MONITOR){
									...
									if (!monitor.MOP_terminated)
										monitor.endObject(valuePattern[j].idnum);
								} else if(valuePattern[j].type == MOPMultiMapSignature.MAP_OF_SET){
									MOPSet set = (MOPSet) values[j];
