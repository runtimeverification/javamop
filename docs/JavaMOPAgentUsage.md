...
3. All the monitor libraries (a number of .java files) generated by
RV-Monitor should be contained in a single directory (see the example
below).
...
4. (Optional) Compile the monitor library file and remove Java file:

	```javac classes/mop/MultiSpec_1RuntimeMonitor.java
	rm classes/mop/MultiSpec_1RuntimeMonitor.java```

5. Build a javamop agent which does not contain AspectJ Weaver and
RV-Monitor jars:
